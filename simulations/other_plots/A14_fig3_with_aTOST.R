...
if (clusterUse==F) {
  ...
  for (i in 1:length(variab_)) {
    for (j in 1:3) {
      ...
      load(file=paste0(load_dir, "/pow_emp_alphaTOST.rdata"))
      ...
      lines(theta_, 100*pow_emp_TOST[1,i,j,], lwd = 2, col = alpha(color[1], 0.75), lty = 1)
      lines(theta_, 100*pow_emp_alphaTOST[1,i,j,], lwd = 2, col = alpha(color[2], 0.75), lty = 1)
      lines(theta_, 100*pow_emp_xTOST[1,i,j,], lwd = 2, col = alpha(color[3], 0.75), lty = 1)
      sim_setting = 3
      load_dir  = paste0(YOUR_PATH, "/mvt_setting/sim_HPC_out/sim_setting_", sim_setting)
      load(file=paste0(load_dir, "/pow_emp_TOST.rdata"))
      load(file=paste0(load_dir, "/pow_emp_alphaTOST.rdata"))
      ...
      if ((i==length(variab_))&&(j==2)){
        ...
        myorder = c(1, 4, 2, 5, 3, 6)
        legend("bottomleft",
               c(paste0(est_names, " ($K=2$)"), paste0(est_names, " ($K=4$)"))[myorder],
               col = c(alpha(color[1:3], 0.75), color[1:3])[myorder],
               lty = c(rep(1, 3), rep(2, 3))[myorder],
