...
## MC & Cluster
clusterUse=T
if(clusterUse==F) destinationDir = "power_curves/sim_HPC_out_power_curves" else destinationDir <- "sim_HPC_out_power_curves"
...
for(rw in Rw){
  ...
  for(u in 1:length(nu_)){
    ...
    for(s in 1:length(sigma_)){
      ...
      # 3. xTOST bootstrap solution
      c0_boot_star = xtost(sig_hat = sig_hat, nu = nu, alpha = alpha, delta = delta, correction = "bootstrap", B = 10^5)
      ...
      for(k in 1:length(theta_)){
        ...
        # 3. xTOST bootstrap BE
        BE_xTOST_boot_[u,s,k] = abs(theta_hat) < c0_boot_star$c
